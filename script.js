    ...
    shopItems.forEach(function(that){
      if(that["sold"] == true) {
        ...
        $(".archive").children().last().click(function(){
          ...
          $("body").append(`<div class="shop-view"><img class="shop-view-image" src="images/shop/${that["images"][0]}"><div class="shop-thumbnails"></div><div class="shop-view-info"><h2>${that["name"]}</h2><p>size: ${that["size"]}</p><p>materials: ${that["materials"]}</p><p>price: Not available</p><p class="rule">Contact brent.e.hildebrand@gmail.com for any inquiries.</p></div><div class="exit"></div></div>`);
